...
sub aggregate_vcfs {
	...
	while ( my $var = $vcf->next_var() ) {
        #Which variantcaller
        my $vc = which_variantcaller( $var->{INFO} );
...
sub fix_gt {
    ...
    $var->{FORMAT} = [];
    ## Prioritize manta values, this comes from SVDB merge. 
    ## Genotype field formatted by the caller with highest priority, thus if called by manta it will have alt ref counts formated by manta
    if( $vc =~ /^(manta)/ ) {
        for my $gt ( @{$var->{GT}} ) {
    ...
    ## If merged callset from SVDB does not have manta, next in prio is delly as it has ref alt counts and could be precisely called breakpoints
    elsif( $vc =~ /^(delly)/) {
    ...
    ## Special rules for MELT, uncommon variants, to make them appear under normal circumstances in coyote VAF 50% and 100 counts alt ref
	elsif( $vc eq "MELT") {
		if ($opt{paired} eq 'paired') {
			my %tmp = ( 'GT' => '0/0');
...
sub which_variantcaller{
    my $set = shift;
    ## svdb software merged SVs gets tag "set" in INFO-field. Intersection=called by all and so on.
    if ($set->{set}) {
        return "manta|delly|cnvkit" if $set->{set} =~ /Intersection/;
        return "manta" if $set->{set} =~ /manta$/;
        return "manta|delly" if $set->{set} =~ /manta\-delly/;
        return "delly" if $set->{set} =~ /delly$/;
        return "delly|cnvkit" if $set->{set} =~ /delly\-cnvkit/;
        return "CNVkit" if $set->{set} =~ /cnvkit/;
    }
    ## special rule from constiutional pipeline
    elsif ( $set->{SCOUT_CUSTOM}) {
        return "MELT";
    }
